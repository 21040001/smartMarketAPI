...
public interface DALStoresInterface extends JpaRepository<Stores, Long> {
    ...
    @Transactional
    @Modifying
    @Query(value = "Update Stores SET chat_id =:chatId WHERE store_id = :storeId", nativeQuery = true)
    void updateChatId(@Param("storeId") int store_id , @Param("chatId") long chatId);
    
    @Transactional
    @Query(value = "SELECT s.chat_id FROM Stores s WHERE s.store_id = :storeId", nativeQuery = true)
    Long getChatId(@Param("storeId") int store_id);
}
